...
export const count = <A>(sa: Stream<A>): Stream<number> => keepIndex(withCount(sa))

// Pair event values with their associated 1-based count.
export const withCount = <A>(sa: Stream<A>): Stream<[number, A]> => withIndexStart(1, sa)

// Replace event values with a 0-based index.
export const index = <A>(sa: Stream<A>): Stream<number> => keepIndex(withIndex(sa))

// Pair event values with their associated 0-based index.
export const withIndex = <A>(sa: Stream<A>): Stream<[number, A]> => withIndexStart(0, sa)
...
export const keepIndex = <I>(s: Stream<[I, unknown]>): Stream<I> => map(ia => ia[0], s)
